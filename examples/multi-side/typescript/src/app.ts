...
async function startScan(sdk: BlinkIDSDK.WasmSDK) {
    ...
    try {
        ...
        async (recognitionState: BlinkIDSDK.RecognizerResultState) => {
            ...
            const firstName = (result.firstName.latin ||
                result.firstName.cyrillic ||
                result.firstName.arabic) || result.mrz.secondaryID;
            const lastName = (result.lastName.latin ||
                result.lastName.cyrillic ||
                result.lastName.arabic) || result.mrz.primaryID;
            const fullName = (result.fullName.latin ||
                result.fullName.cyrillic ||
                result.fullName.arabic) || `${result.mrz.secondaryID} ${result.mrz.primaryID}`;
            const dateOfBirth = {
