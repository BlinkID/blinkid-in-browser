...
export class MbComponent {
    ...
    const eventHandler = (recognitionEvent: RecognitionEvent) => {
      switch (recognitionEvent.status) {
        ...
        case RecognitionStatus.BarcodeScanningStarted:
          this.cameraExperience
            .setState(
              CameraExperienceState.BarcodeScanning,
              this.isBackSide,
              true,
            )
            .then(() => {
  ...
  private handleScanError(error: any) {
    ...
    if (error?.code === ErrorCodes.INVALID_PING_PROXY_URL) {
      this.setFatalError(
        new SDKError(
          ErrorTypes.componentErrors.pingProxyErrors.invalidProxyUrl,
          error,
        ),
      );
      ...
      this.setFatalError(
        new SDKError(
          ErrorTypes.componentErrors.pingProxyErrors.permissionNotGranted,
          error,
        ),
      );
